...
- Hard-fail release signoff if browser evidence is missing.
...
- Every required `integration_flows[*].flow_id` must appear in `harnass-os/documents/audit/browser/current.yaml` with `status: passed`.
- Any browser `console_errors` or `network_failures` are blocking for UI production signoff.
- `stateful` flows require an explicit audit account reference; otherwise the module is blocking.
