...
INCLUDE_UNTRACKED_MARKER=".claude/.atomic_include_untracked"
...
if [[ -f "$INCLUDE_UNTRACKED_MARKER" ]]; then
  git add -A
  echo "[AtomicCommit] Staging tracked + untracked changes (opt-in marker found)."
else
  git add -u
  echo "[AtomicCommit] Staging tracked changes only (default mode)."
fi

if git diff --cached --quiet; then
  echo "[AtomicCommit] No staged changes after applying staging policy; checkpoint skipped."
  rm -f "$MARKER" >/dev/null 2>&1 || true
  exit 0
fi
