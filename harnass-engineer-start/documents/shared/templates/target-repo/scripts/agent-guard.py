...
UI_BROWSER_MODULES = {"frontend-experience", "frontend-quality", "frontend-backend-integration", "accessibility", "seo"}
...
def browser_audit_report() -> dict[str, object]:
    path = DOCS / "audit" / "browser" / "current.yaml"
    return read_optional_yaml(path)
...
def validate_milestones(data: dict[str, object], task_ids: set[str]) -> list[str]:
    ...
    for milestone_obj in raw_milestones:
        ...
        execution_context = milestone_obj.get("execution_context")
        if execution_context not in {"milestone-worktree", "branch-only"}:
            errors.append(f"milestone {milestone_id}: execution_context must be milestone-worktree or branch-only")
        if execution_context == "milestone-worktree":
            if non_placeholder_string(milestone_obj.get("worktree_ref")) is None:
                errors.append(f"milestone {milestone_id}: milestone-worktree requires worktree_ref")
            if non_placeholder_string(milestone_obj.get("branch_ref")) is None:
                errors.append(f"milestone {milestone_id}: milestone-worktree requires branch_ref")
...
def validate_audit_module_results(module_results: list[object], *, require_passed: bool) -> list[str]:
    ...
    for index, raw_module in enumerate(module_results):
        ...
        if not isinstance(module.get("browser_evidence_refs"), list):
            errors.append(f"audit {module_id}: browser_evidence_refs must be a list")
        if not isinstance(module.get("network_assertions"), list):
            errors.append(f"audit {module_id}: network_assertions must be a list")
        if not isinstance(module.get("console_assertions"), list):
            errors.append(f"audit {module_id}: console_assertions must be a list")
        ...
        if require_passed and applicable is True:
            ...
            if module_id in UI_BROWSER_MODULES and not as_list(module.get("browser_evidence_refs")):
                errors.append(f"audit {module_id}: UI module must include browser_evidence_refs when audit passes")
    return errors
...
def final_audit_pass_errors(audit: dict[str, object]) -> list[str]:
    ...
    if inventory.get("ui_facing") is True:
        browser_report = browser_audit_report()
        if not browser_report:
            errors.append("audit: UI release requires harnass-os/documents/audit/browser/current.yaml")
        elif browser_report.get("status") != "passed":
            errors.append("audit: browser audit report must be passed for UI release signoff")
        elif audit.get("browser_report_ref") != "harnass-os/documents/audit/browser/current.yaml":
            errors.append("audit: browser_report_ref must point to harnass-os/documents/audit/browser/current.yaml")
    return errors
...
def normalize_execution_plan_shape(data: dict[str, object]) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    tasks: list[dict[str, object]] = []
    milestones: list[dict[str, object]] = []
    for raw_task in as_list(data.get("tasks")):
        task = dict(as_mapping(raw_task))
        task.pop("status", None)
        task.pop("handoff_output", None)
        tasks.append(task)
    for raw_milestone in as_list(data.get("milestones")):
        milestone = dict(as_mapping(raw_milestone))
        milestone.pop("status", None)
...
def command_record_task_change(_: list[str]) -> int:
    ...
    previous_tasks, previous_milestones = normalize_execution_plan_shape(previous)
    current_tasks, current_milestones = normalize_execution_plan_shape(data)
    task_shape_changed = previous_tasks != current_tasks or previous_milestones != current_milestones
...
def command_check_final_audit_ready(_: list[str]) -> int:
    ...
    if deploy_state not in {"deployed", "production-passed", "rollback_required"}:
    ...
    if inventory.get("ui_facing") is True and not browser_audit_report():
        raise GuardFailure(
            "final audit is blocked until harnass-os/documents/audit/browser/current.yaml exists for UI releases", 1
        )
    return 0
...
def command_check_task_fields(_: list[str]) -> int:
    ...
    for task in tasks:
        ...
        raw_flows = task.get("integration_flows")
        if release_plan and ui_plan and raw_flows is None:
            errors.append(f"task {tid}: UI release task must define integration_flows")
        if raw_flows is not None:
            for index, flow in enumerate(as_list(raw_flows), 1):
                if not isinstance(flow, dict):
                    errors.append(f"task {tid}: integration_flows[{index}] must be an object")
                    continue
                if non_placeholder_string(flow.get("flow_id")) is None:
                    errors.append(f"task {tid}: integration_flows[{index}].flow_id is missing or placeholder")
                if non_placeholder_string(flow.get("path")) is None:
                    errors.append(f"task {tid}: integration_flows[{index}].path is missing or placeholder")
                if flow.get("mode") not in {"read_only", "stateful"}:
                    errors.append(f"task {tid}: integration_flows[{index}].mode must be read_only or stateful")
                if not as_list(flow.get("steps")):
                    errors.append(f"task {tid}: integration_flows[{index}].steps must be a non-empty list")
                if not as_list(flow.get("expected_requests")):
                    errors.append(f"task {tid}: integration_flows[{index}].expected_requests must be a non-empty list")
                if not as_list(flow.get("pass_when")):
                    errors.append(f"task {tid}: integration_flows[{index}].pass_when must be a non-empty list")
                if flow.get("mode") == "stateful" and non_placeholder_string(flow.get("audit_account_ref")) is None:
                    errors.append(
                        f"task {tid}: integration_flows[{index}] is stateful and must define audit_account_ref"
                    )
